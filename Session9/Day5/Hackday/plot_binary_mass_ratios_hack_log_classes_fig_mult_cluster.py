...
for fname_df in fname_dfs:
    df_tot=pd.read_csv("{}/{}".format(path,fname_df),sep="\t",index_col=0) # orbits post selection

    # only plot the simple binary systems
    df_bin=df_tot[df_tot['N_sys']==2]
    df_mult=df_tot[df_tot['N_sys']>2]
    ...
    df_cluster_fit=df_tot[df_tot['N_sys']>=2]

    df_kde=df_cluster_fit.sort_values(by=['m2/m1'],ascending=False)
    print len(df_kde)
    ...
    for i,k in enumerate(unique_labels_order):
        ...
        if k==-1:
            ...
            for simplex in hullPoints.simplices:
                ...
                color=cont_cols[i],label=label,zorder=0,dashes=cluster_linestyle[i])
    ...
    # cluster members
    df_cluster=df_fit[core_samples_mask]
