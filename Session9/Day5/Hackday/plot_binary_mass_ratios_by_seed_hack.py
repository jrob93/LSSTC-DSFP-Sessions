...
for fname_df in fname_dfs:
    ...
    print list(df)
    print df['initial'].to_string()
    df['seed']=[s.split("/")[-1] for s in df['initial']]
    print df['seed']

    seed_vals = numpy.unique(numpy.array(df['seed']))
    ...
    for j,s in enumerate(seed_vals):
        df2=df[df['seed']==s]
    ...
    for i,s in enumerate(seed_vals):
        ...
        # label plots
        f_label="{}. seed={}".format(plot_label[i],s)
