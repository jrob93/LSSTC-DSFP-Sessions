...
ax5.scatter(X[:,0],X[:,1],edgecolors="k",facecolors='none')
...
for i,k in enumerate(unique_labels):
    ...
    if k==-1:
        ...
        X_dat=numpy.array(df_cluster['x'])
        Y_dat=numpy.array(df_cluster['y'])
        Z_dat=numpy.zeros(len(X_dat))+k
...
ax1.plot(grid_x,pf.line_fit(grid_x,slope,intercept),c='k')
...
class_member_mask = ((labels == 1) | (labels == 2))
# cluster members
df_cluster=df_fit[class_member_mask & core_samples_mask]
N_obs=len(df_cluster)
# ax1.scatter(df_cluster['x'],df_cluster['y'],s=10,c='k')
...
ax1.plot(numpy.log10(mass_ratios),numpy.log10(m2_min+m1_min),c='k',linestyle=":")
ax4.plot(mass_ratios,m2_min+m1_min,c='k',linestyle=":")
#-------------------------------------------------------------------------------
# Add contours to data

triang = tri.Triangulation(x, y)
ax1.tricontour(X_dat, Y_dat, Z_dat, 15, linewidths=0.5, colors='k')
ax1.tricontourf(X_dat, Y_dat, Z_dat, 15)
...
ax5.set_xlabel('scaled x')
ax5.set_ylabel("scaled y")
