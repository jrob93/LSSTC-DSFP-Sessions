...
for fname_df in fname_dfs:
    ...
    fig.text(0.5, 0.01, '$\\log(m_2/m_1)$', ha='center')
    fig.text(0.0, 0.5, '$\\log((m_2+m_1)/M_{{\\mathrm{{c}}}})$', va='center', rotation='vertical')
    ...
    for j,f in enumerate(f_vals):
        ...
        print "f={}, number of data points = {}".format(f,len(x_data))
        
        for i in range(len(x_data)):
            label="M_c={:.2e}kg".format(Mtot[i])
            axes[j].scatter(numpy.log10(x_data[i]),numpy.log10(y_data[i]),
    ...
    for i,f in enumerate(f_vals):
        ...
        # if i%2!=0:
        #     axes[i].text(1.0,1.0-padding,f_label,fontdict={'size': 6},ha='right',transform=axes[i].transAxes)
        # else:
        #     axes[i].text(0.0,1.0-padding,f_label,fontdict={'size': 6},ha='left',transform=axes[i].transAxes)
        if (i==0) or (i==1):
            axes[i].text(0.0+padding,1.0-padding,f_label,fontdict={'size': 6},ha='left',va='top',transform=axes[i].transAxes)
        else:
            axes[i].text(0.0+padding,0.0,f_label,fontdict={'size': 6},ha='left',va='bottom',transform=axes[i].transAxes)
        ...
        #
        # set axis limits
        # axes[i].set_xlim(-3.25-padding,0.0+(2.0*padding))
        # axes[i].set_ylim(-4.25-padding,0.0+(2.0*padding))
        axes[i].set_xlim(-3.3,0.25)
        axes[i].set_ylim(-4.75,0.25)

        axes[i].xaxis.set_ticks(numpy.arange(-3,1))
    ...
    # ax1.legend(by_label.values(), by_label.keys(),loc='lower left',prop={'size': 6},mode="expand",ncol=3)

    # h, l = ax1.get_legend_handles_labels()
    # ax5.legend(h, l,mode="expand",ncol=3)
