...
for fname_df in fname_dfs:
    ...
    for i,M_tot in enumerate(numpy.unique(numpy.array(df['M_tot(kg)']).astype(float))):
        ...
        ax1.scatter(numpy.log10((m1+m2)/Mtot),numpy.log10(m2m1),
    ...
    x_data=numpy.log10((m1+m2)/Mtot)
    y_data=numpy.log10(m2m1)
    ...
    grid_x=numpy.linspace(x_min,0.0)
    ...
    area = simps(PDF_y, dx=numpy.diff(grid_x)[0])
    print("area =", area)
